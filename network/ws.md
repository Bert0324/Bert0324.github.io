...
Q: 关于帧和流的区别，为什么ws使用帧而不是流？[stackoverflow](https://stackoverflow.com/questions/48842357/frame-based-and-stream-based-protocols)有一个回答，但我觉得不是很清晰也没看懂。
...
实际上，不止B站，斗鱼和虎牙的ws连接也是这样的，用`Binary Message`作为统一的事件名，内容用zlib压缩，比如以下的一条消息：
...
Q: 他们为什么采用了这种形式？想到的好处可能有cpu换带宽，方便事件名管理。
